...
**[Join the Waitlist for Syfra Cloud](https://ancora.co/waitlist)**
...
ancora save "Fixed N+1 query" "Optimized user list query" --workspace=myapp --type=bugfix

# Save personal memory
ancora save "2026 Goals" "my goals for the year" --visibility=personal
...
ancora export backup.json
ancora import backup.json
```
...
- **HTTP API** - Ancora REST endpoints on port 7437
...
```
cmd/ancora/           # CLI entrypoint with subcommands
internal/
  ...
  server/            # HTTP API server exposed on port 7437
...
ancora save "Migrated to bcrypt from SHA256" \
...
ancora save "Health Goals 2026" "my health goals: lose 10 lbs, run 5k" --workspace=life

# Later: search for past decisions
ancora search "password hashing" --type=decision --workspace=webapp

# View timeline context around observation #127
ancora timeline 127 --before=5 --after=5
```
