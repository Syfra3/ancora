...
import (
	...
	ancorasrv "github.com/Syfra3/ancora/internal/server"
...
func stubRuntimeHooks(t *testing.T) {
	...
	storeNew = store.New
	newHTTPServer = func(s *store.Store, _ int) *ancorasrv.Server { return ancorasrv.New(s, 0) }
	startHTTP = func(_ *ancorasrv.Server) error { return nil }
...
func TestCmdServeParsesPortAndErrors(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			seenPort := -1
			newHTTPServer = func(s *store.Store, port int) *ancorasrv.Server {
				seenPort = port
				return ancorasrv.New(s, 0)
			}
			startHTTP = func(_ *ancorasrv.Server) error {
				return tc.startErr
			}
