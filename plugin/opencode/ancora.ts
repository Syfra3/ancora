/**
 * Ancora — OpenCode plugin adapter
 *
 * Thin layer that connects OpenCode's event system to the Ancora Go binary.
 * The Go binary runs as a local HTTP server and handles all persistence.
 *
 * Flow:
 *   OpenCode events → this plugin → HTTP calls → ancora serve → SQLite
 *
 * Session resilience:
...
const ANCORA_PORT = parseInt(process.env.ANCORA_PORT ?? "7437")
const ANCORA_URL = `http://127.0.0.1:${ANCORA_PORT}`
...
async function ancoraFetch(
  path: string,
  opts: { method?: string; body?: any } = {}
): Promise<any> {
  try {
    const res = await fetch(`${ANCORA_URL}${path}`, {
    ...
    // Ancora server not running — silently fail
    return null
  }
}

async function isAncoraRunning(): Promise<boolean> {
  try {
    const res = await fetch(`${ANCORA_URL}/health`, {
...
/**
 * Strip <private>...</private> tags before sending to ancora.
...
export const Ancora: Plugin = async (ctx) => {
  ...
  // Track which sessions we've already ensured exist in ancora
  ...
  // We must not register them as top-level Ancora sessions — they cause session
  // inflation (e.g. 170 sessions for 1 real conversation, issue #116).
  const subAgentSessions = new Set<string>()

  /**
   * Ensure a session exists in ancora. Idempotent — calls POST /sessions
  ...
  async function ensureSession(sessionId: string): Promise<void> {
    ...
    // Do not register sub-agent sessions in Ancora (issue #116).
    if (subAgentSessions.has(sessionId)) return
    knownSessions.add(sessionId)
    await ancoraFetch("/sessions", {
      method: "POST",
      body: {
        id: sessionId,
        project,
  ...
  // Try to start ancora server if not running
  const running = await isAncoraRunning()
  if (!running) {
    try {
      Bun.spawn([ANCORA_BIN, "serve"], {
        stdout: "ignore",
        stderr: "ignore",
        stdin: "ignore",
      })
  ...
  if (oldProject !== project) {
    await ancoraFetch("/projects/migrate", {
      method: "POST",
      body: { old_project: oldProject, new_project: project },
    })
  }

  // Auto-import: if .ancora/manifest.json exists in the project repo,
  // run `ancora sync --import` to load any new chunks into the local DB.
  ...
  try {
    const manifestFile = `${ctx.directory}/.ancora/manifest.json`
    const file = Bun.file(manifestFile)
    if (await file.exists()) {
      Bun.spawn([ANCORA_BIN, "sync", "--import"], {
        cwd: ctx.directory,
        stdout: "ignore",
        stderr: "ignore",
        stdin: "ignore",
  ...
  return {
    ...
    event: async ({ event }) => {
      ...
      if (event.type === "session.created") {
        ...
        // top-level Ancora sessions. They cause massive session inflation
    ...
    "tool.execute.after": async (input, output) => {
      if ( ANCORA_TOOLS.has(input.tool.toLowerCase())) return
      ...
      if (input.tool === "Task" && output && sessionId) {
        ...
        if (text.length > 50) {
          await ancoraFetch("/observations/passive", {
            method: "POST",
            body: {
              session_id: sessionId,
    ...
    // This ensures the agent ALWAYS knows about Ancora, even after compaction.
    //
    ...
    "experimental.session.compacting": async (input, output) => {
      ...
      // compacted summary to Ancora. The new agent reads the compacted
