#!/bin/bash
# Ancora — UserPromptSubmit hook for Claude Code
#
...
if [ -n "$SESSION_ID" ]; then
  SESSION_KEY="ancora-claude-${SESSION_ID}-tools-loaded"
else
  ...
  SESSION_KEY="ancora-claude-${SAFE_PROJECT}-$$-tools-loaded"
fi
...
SESSION_START=""
if [ -n "$SESSION_ID" ]; then
  SESSION_START=$(curl -sf "${ANCORA_URL}/sessions/${SESSION_ID}" --max-time 0.2 2>/dev/null \
...
LAST_SAVE_JSON=$(curl -sf \
  "${ANCORA_URL}/observations?project=${ENCODED_PROJECT}&limit=1&sort=created_at:desc" \
