---
name: ancora-memory
description: "ALWAYS ACTIVE — Persistent memory protocol. You MUST save decisions, conventions, bugs, and discoveries to Ancora proactively. Do NOT wait for the user to ask."
---

# Ancora Persistent Memory — Protocol
...
- `ancora_get`, `ancora_suggest_topic`, `ancora_update`
- `ancora_start`, `ancora_end`, `ancora_save_prompt`
...
- `ancora_stats`, `ancora_delete`, `ancora_timeline`, `ancora_capture`

## PROACTIVE SAVE TRIGGERS (mandatory — do NOT wait for user to ask)

Call `ancora_save` IMMEDIATELY and WITHOUT BEING ASKED after any of these:
...
- **project** (optional): Project/repo name (e.g. `glim-api`, `syfra`, `life`)
- **scope** (optional): `project` (default) | `personal`
  - `project` — Work knowledge: coding, research, architecture, business. Sync-eligible if project enrolled.
  - `personal` — Private life knowledge (health, finances, goals). NEVER synced to Syfra Cloud.
  - **Auto-detection**: If omitted, scope is inferred from content. Personal triggers: "my goals", "my health", "my finances", "personal", "private", "family", etc.
...
- If the same topic evolves, call `ancora_save` with the same `topic_key` so memory is updated (upsert) instead of creating a new observation
- If unsure about the key, call `ancora_suggest_topic` first, then reuse that key consistently
- If you already know the exact ID to fix, use `ancora_update`
