...
func TestObservationDetailEscKey(t *testing.T) {
	fx := newTestFixture(t)
	m := New(fx.store, "")
	m.Screen = ScreenObservationDetail
	...
	// Test ESC key goes back to previous screen
	updatedModel, cmd := m.handleObservationDetailKeys("esc")
	updated := updatedModel.(Model)
	if updated.Screen != ScreenRecent {
		t.Fatalf("ESC should go to PrevScreen (Recent), got %v", updated.Screen)
	}
	if cmd == nil {
		t.Fatal("ESC should trigger refresh command")
	}
	if updated.DetailScroll != 0 {
		t.Fatal("ESC should reset DetailScroll")
	}
	if updated.Cursor != 0 {
		t.Fatal("ESC should reset Cursor")
	}
	...
	m.DetailScroll = 5
	updatedModel, _ = m.handleObservationDetailKeys("q")
	updated = updatedModel.(Model)
	if updated.Screen != ScreenSearchResults {
		t.Fatalf("q should go to PrevScreen (SearchResults), got %v", updated.Screen)
	}
	if updated.DetailScroll != 0 {
		t.Fatal("q should reset DetailScroll")
	}
}

func TestObservationDetailMoveKeys(t *testing.T) {
	fx := newTestFixture(t)
	proj := "ancora"
	m := New(fx.store, "")
	m.Screen = ScreenObservationDetail
	...
	updated := updatedModel.(Model)
	if updated.Screen != ScreenMoveObservation {
		t.Fatal("m key should open move screen")
	}
	if updated.MoveObservationID != fx.obsID {
		t.Fatalf("move observation ID = %d, want %d", updated.MoveObservationID, fx.obsID)
	}
	if updated.MoveScopeCursor != 0 {
		t.Fatal("scope cursor should start at 0 (project)")
	}
	if updated.MoveActiveColumn != "project" {
		t.Fatal("should start with project column active")
	}
	if cmd == nil {
		t.Fatal("opening move screen should trigger loadMoveProjects command")
	}

	// Simulate projects loaded
	m.Screen = ScreenMoveObservation
	m.MoveProjectList = []string{"syfra", "ancora", "ancora"}
	m.MoveProjectCursor = 1
	m.MoveActiveColumn = "project"

	// Test Tab key switches columns
	updatedModel, _ = m.handleMoveObservationKeys("tab")
	updated = updatedModel.(Model)
	if updated.MoveActiveColumn != "scope" {
		t.Fatal("tab should switch to scope column")
	}

	// Test down key moves scope cursor when scope is active
	updatedModel, _ = updated.handleMoveObservationKeys("down")
	updated = updatedModel.(Model)
	if updated.MoveScopeCursor != 1 {
		t.Fatal("down should move scope cursor to 1 (personal)")
	}

	// Test up key moves scope cursor back
	updatedModel, _ = updated.handleMoveObservationKeys("up")
	updated = updatedModel.(Model)
	if updated.MoveScopeCursor != 0 {
		t.Fatal("up should move scope cursor back to 0")
	}

	// Test Tab switches back to project column
	updatedModel, _ = updated.handleMoveObservationKeys("tab")
	updated = updatedModel.(Model)
	if updated.MoveActiveColumn != "project" {
		t.Fatal("tab should switch back to project column")
	}

	// Test down key moves project cursor when project is active
	m.MoveProjectCursor = 0
	m.MoveActiveColumn = "project"
	updatedModel, _ = m.handleMoveObservationKeys("down")
	updated = updatedModel.(Model)
	if updated.MoveProjectCursor != 1 {
		t.Fatal("down should move project cursor to 1")
	}

	// Test Enter saves both project and scope
	updatedModel, cmd = updated.handleMoveObservationKeys("enter")
	updated = updatedModel.(Model)
	if cmd == nil {
		t.Fatal("enter should return move command")
	}

	// Test ESC goes back to observation detail
	updatedModel, _ = m.handleMoveObservationKeys("esc")
	updated = updatedModel.(Model)
	if updated.Screen != ScreenObservationDetail {
		t.Fatal("esc should return to observation detail")
	}

	m.Screen = ScreenMoveObservation
	m.MoveDone = true
	updatedModel, cmd = m.handleMoveObservationKeys("esc")
	updated = updatedModel.(Model)
	if updated.Screen != ScreenObservationDetail {
		t.Fatal("esc after done should return to detail")
	}
	if cmd == nil {
		t.Fatal("esc after done should reload observation detail")
	}
}

func TestMoveObservationMessageHandling(t *testing.T) {
	m := New(nil, "")

	updatedModel, _ := m.Update(observationMovedMsg{err: errors.New("move failed")})
	updated := updatedModel.(Model)
	if updated.MoveError != "move failed" {
		t.Fatalf("move error = %q", updated.MoveError)
	}

	updatedModel, _ = m.Update(observationMovedMsg{err: nil})
	updated = updatedModel.(Model)
	if !updated.MoveDone {
		t.Fatal("successful move should set MoveDone")
	}
	if updated.MoveError != "" {
		t.Fatal("successful move should clear MoveError")
	}
}

func TestHandleProjectsKeysWithMoveFeature(t *testing.T) {
	fx := newTestFixture(t)
	m := New(fx.store, "")
	m.Height = 12
	m.Screen = ScreenProjects
	m.Projects = []ProjectWithEnrollment{
		{ProjectStats: store.ProjectStats{Name: "ancora", ObservationCount: 10, SessionCount: 2}},
		{ProjectStats: store.ProjectStats{Name: "syfra", ObservationCount: 5, SessionCount: 1}},
	}
	m.Cursor = 0
	m.Scroll = 0

	updatedModel, _ := m.handleProjectsKeys("down")
	updated := updatedModel.(Model)
	if updated.Cursor != 1 {
	...
	updated.Cursor = 0
	updatedModel, cmd := m.handleProjectsKeys("enter")
	updated = updatedModel.(Model)
	if updated.Screen != ScreenRecent {
