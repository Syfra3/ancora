// Package store implements the persistent memory engine for Ancora.
//
// It uses SQLite with FTS5 full-text search to store and retrieve
// observations from AI coding sessions. This is the core of Ancora —
...
// ExportData is the full serializable dump of the Ancora database.
...
func DefaultConfig() (Config, error) {
	...
	if err != nil {
		return Config{}, fmt.Errorf("ancora: determine home directory: %w", err)
	}
	return Config{
		DataDir:              filepath.Join(home, ".ancora"),
...
func New(cfg Config) (*Store, error) {
	if !filepath.IsAbs(cfg.DataDir) {
		return nil, fmt.Errorf("ancora: data directory must be an absolute path, got %q — set ANCORA_DATA_DIR or ensure your home directory is resolvable", cfg.DataDir)
	}
	if err := os.MkdirAll(cfg.DataDir, 0755); err != nil {
		return nil, fmt.Errorf("ancora: create data dir: %w", err)
	}
	...
	for _, p := range pragmas {
		if _, err := db.Exec(p); err != nil {
			return nil, fmt.Errorf("ancora: pragma %q: %w", p, err)
		}
	}
...
func (s *Store) migrate() error {
	...
	// Vector embeddings — stored as BLOB (float32 LE bytes, 768-dim).
	// Added in Ancora Phase 1. No CGO required — pure modernc.org/sqlite.
	if err := s.addColumnIfNotExists("observations", "embedding", "BLOB"); err != nil {
		return err
	}
...
func (s *Store) UpdateObservation(id int64, p UpdateObservationParams) (*Observation, error) {
	...
	err := s.withTx(func(tx *sql.Tx) error {
		...
		return s.enqueueObservationSyncTx(tx, updated)
	})
	if err != nil {
		return nil, err
	}
...
func (s *Store) SearchSemantic(queryVec []float32, limit int) ([]SearchResult, error) {
	...
	for rows.Next() {
		...
		candidates = append(candidates, candidate{sr, sim})
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	...
	results := make([]SearchResult, 0, min(limit, len(candidates)))
	for i := 0; i < limit && i < len(candidates); i++ {
		sr := candidates[i].SearchResult
		sr.Rank = float64(candidates[i].sim)
		results = append(results, sr)
	}
	return results, nil
}

// serializeVec encodes []float32 as little-endian bytes.
...
// deserializeVec decodes little-endian bytes back to []float32.
...
// normalizeScope enforces the two-scope model:
//
//   - "personal" — private life domain (finance, health, goals, etc.).
//     These observations are stored locally ONLY and are NEVER enqueued
//     for Syfra Cloud sync, regardless of project enrollment.
//
//   - "project" (default) — any project: coding repos, research, writing,
//     business plans, side projects, etc. Sync eligibility is controlled
//     exclusively by project enrollment (sync_enrolled_projects table), not
//     by this scope value.
//
// Any unrecognised value (empty string, "org", legacy values) falls back to
// "project" so old data and typos degrade gracefully.
