...
//     (patching ANCORA_BIN to bake in the absolute binary path as a final
...
//     then writes a durable MCP config to ~/.claude/mcp/ancora.json using the
...
// Command is always the bare "ancora" name in this constant because
// upsertCodexAncorBlock generates the actual content via codexAncorBlockStr()
// which uses resolveAncorCommand() at runtime. This constant is kept for tests
// that verify idempotency against the already-written string when os.Executable
// returns "ancora" (fallback path).
const codexAncorBlock = "[mcp_servers.ancora]\ncommand = \"ancora\"\nargs = [\"mcp\", \"--tools=agent\"]"

// codexAncorBlockStr returns the Codex TOML block for the ancora MCP server,
// using the resolved absolute binary path from os.Executable().
func codexAncorBlockStr() string {
	cmd := resolveAncorCommand()
	return "[mcp_servers.ancora]\ncommand = " + fmt.Sprintf("%q", cmd) + "\nargs = [\"mcp\", \"--tools=agent\"]"
}

const memoryProtocolMarkdown = `## Ancora Persistent Memory — Protocol
...
with numbered items. Ancora will automatically extract and save these as observations.
...
const codexCompactPromptMarkdown = `You are compacting a coding session that uses Ancora persistent memory.
...
func ValidateBinaryInPATH() (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf(
			"ancora binary %q not found in PATH.\n"+
				"Add the directory containing the binary to your PATH, then restart setup.\n"+
				"Typical install locations: ~/.local/bin, /usr/local/bin, ~/go/bin",
			binaryName,
		)
	}
	return path, nil
...
// SupportedAgents returns the list of agents supported by ancora in Phase 1.
// Only Claude Code and OpenCode are supported. Gemini CLI and Codex are
...
// patchAncorBINLine rewrites the ANCORA_BIN constant declaration in the
// plugin source so the installed copy contains an absolute fallback path.
//
// Original line in source:
//
//	const ANCORA_BIN = process.env.ANCORA_BIN ?? "ancora"
//
// Patched line in installed copy:
//
//	const ANCORA_BIN = process.env.ANCORA_BIN ?? Bun.which("ancora") ?? "/abs/path/ancora"
//
// Priority (left to right, first truthy wins):
//  1. ANCORA_BIN env var — explicit user override, always respected.
//  2. Bun.which("ancora") — runtime PATH lookup; works in interactive shells.
//  3. Absolute baked-in path — works in headless/systemd where PATH is stripped.
//
// If absBin is already bare "ancora" (os.Executable fallback) we don't add it
// as the third fallback because it would be redundant with Bun.which("ancora").
func patchAncorBINLine(src []byte, absBin string) []byte {
	const marker = `const ANCORA_BIN = process.env.ANCORA_BIN ?? "ancora"`

	var replacement string
	if absBin == "ancora" {
		// os.Executable failed — add Bun.which but no baked-in absolute path
		replacement = `const ANCORA_BIN = process.env.ANCORA_BIN ?? Bun.which("ancora") ?? "ancora"`
	} else {
		// Normal case: bake in the absolute path as final fallback
		replacement = fmt.Sprintf(
			`const ANCORA_BIN = process.env.ANCORA_BIN ?? Bun.which("ancora") ?? %q`,
			absBin,
		)
	}
...
func installOpenCode() (*Result, error) {
	...
	data, err := openCodeReadFile("plugins/opencode/ancora.ts")
	if err != nil {
		return nil, fmt.Errorf("read embedded ancora.ts: %w", err)
	}

	// Patch ANCORA_BIN in the installed copy so the plugin can find the binary
	...
	// process.env.ANCORA_BIN (explicit user override) and Bun.which("ancora")
	...
	data = patchAncorBINLine(data, resolveAncorCommand())

	dest := filepath.Join(dir, "ancora.ts")
	if err := openCodeWriteFileFn(dest, data, 0644); err != nil {
		return nil, fmt.Errorf("write %s: %w", dest, err)
	}

	// Register ancora MCP server in opencode.json
	files := 1
	if err := injectOpenCodeMCPFn(); err != nil {
		...
		fmt.Fprintf(os.Stderr, "  \"ancora\": { \"type\": \"local\", \"command\": [%q, \"mcp\", \"--tools=agent\"], \"enabled\": true }\n", cmd)
	} else {
		files = 2
	}
...
// injectOpenCodeMCP adds the Ancora MCP server entry to opencode.json.
// It reads the existing config, adds/updates the ancora entry under "mcp",
...
func injectOpenCodeMCP() error {
	...
	// Check if ancora is already registered
	if _, exists := mcpBlock["ancora"]; exists {
		return nil // already registered, nothing to do
	}

	// Add ancora MCP entry (agent profile — only tools agents need).
	// Use resolveAncorCommand() so Windows users (and headless Linux setups
	// where PATH is not inherited) get the absolute binary path.
	ancorEntry := map[string]interface{}{
		"type":    "local",
		"command": []string{resolveAncorCommand(), "mcp", "--tools=agent"},
		"enabled": true,
	}
	entryJSON, err := jsonMarshalFn(ancorEntry)
	if err != nil {
		return fmt.Errorf("marshal ancora entry: %w", err)
	}
	mcpBlock["ancora"] = json.RawMessage(entryJSON)
...
func installClaudeCode() (*Result, error) {
	...
	installOut, err := runCommand(claudeBin, "plugin", "install", "ancora")
	...
	// Step 3: Write a durable user-level MCP config at ~/.claude/mcp/ancora.json
	...
	if err := writeClaudeCodeUserMCPFn(); err != nil {
		...
		fmt.Fprintf(os.Stderr, "warning: could not write user MCP config (~/.claude/mcp/ancora.json): %v\n", err)
		fmt.Fprintf(os.Stderr, "  The plugin is installed but MCP may not start on Windows if ancora is not in PATH.\n")
	} else {
		files = 1
	}
...
// claudeCodeUserMCPPath returns the path for the ancora MCP config in the
// user-level MCP directory.
func claudeCodeUserMCPPath() string {
	return filepath.Join(claudeCodeMCPDir(), "ancora.json")
}

// writeClaudeCodeUserMCP writes ~/.claude/mcp/ancora.json with the absolute
// path to the ancora binary. This is idempotent — it always writes (overwrites)
...
// AddClaudeCodeAllowlist adds ancora MCP tool names to ~/.claude/settings.json
...
func injectGeminiMCP(configPath string) error {
	...
	ancorEntry := map[string]any{
		"command": resolveAncorCommand(),
		"args":    []string{"mcp", "--tools=agent"},
	}
	entryJSON, err := jsonMarshalFn(ancorEntry)
	if err != nil {
	...
	mcpServers["ancora"] = json.RawMessage(entryJSON)
...
// resolveAncorCommand returns the absolute path to the ancora binary.
...
// Falls back to bare "ancora" only if os.Executable() itself fails.
func resolveAncorCommand() string {
	exe, err := osExecutable()
	if err != nil {
		return "ancora" // fallback to PATH-based name
	}
	if resolved, err := filepath.EvalSymlinks(exe); err == nil {
		exe = resolved
	}
...
// Previous versions of ancora added this line, but it causes Gemini CLI to look
...
func injectCodexMCP(configPath string) error {
	...
	updated := upsertCodexAncorBlock(string(data))
...
func upsertCodexAncorBlock(content string) string {
...
func codexInstructionsPath() string {
	return filepath.Join(filepath.Dir(codexConfigPath()), "ancora-instructions.md")
}

func codexCompactPromptPath() string {
	return filepath.Join(filepath.Dir(codexConfigPath()), "ancora-compact-prompt.md")
}
