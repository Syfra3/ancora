...
func TestSupportedAgentsIncludesOnlyPhase1Agents(t *testing.T) {
	agents := SupportedAgents()

	if len(agents) != 2 {
		t.Fatalf("expected exactly 2 supported agents (opencode, claude-code), got %d", len(agents))
	}

	var hasOpenCode, hasClaudeCode bool
	for _, agent := range agents {
		...
		case "claude-code":
			hasClaudeCode = true
		case "gemini-cli", "codex":
			t.Errorf("unexpected agent in Phase 1: %q (gemini-cli and codex not in scope)", agent.Name)
		}
	}

	if !hasOpenCode {
		t.Fatalf("expected opencode in supported agents")
	}
	if !hasClaudeCode {
		t.Fatalf("expected claude-code in supported agents")
	}
}
...
func TestInstallGeminiCLIInjectsMCPConfig(t *testing.T) {
	...
	ancora, ok := mcpServers["ancora"].(map[string]any)
	if !ok {
		t.Fatalf("expected mcpServers.ancora object")
	}

	// Since resolveAncorCommand() uses os.Executable() on all platforms, the
	// command will be the real test binary path in integration tests (not bare
	// "ancora"). Verify it is a non-empty absolute path.
	cmd, ok := ancora["command"].(string)
	if !ok || cmd == "" {
		t.Fatalf("expected non-empty command string, got %#v", ancora["command"])
	}
	if cmd == "ancora" {
		t.Fatalf("expected absolute path from os.Executable(), got bare 'ancora'")
	}

	args, ok := ancora["args"].([]any)
	if !ok || len(args) != 2 || args[0] != "mcp" || args[1] != "--tools=agent" {
		t.Fatalf("expected args [mcp --tools=agent], got %#v", ancora["args"])
	}
	...
	if _, err := installGeminiCLI(); err != nil {
...
func TestInstallCodexInjectsTOMLAndIsIdempotent(t *testing.T) {
	...
	readAndAssert := func() string {
		...
		if strings.Count(text, "[mcp_servers.ancora]") != 1 {
			t.Fatalf("expected exactly one ancora section, got:\n%s", text)
		}
		// resolveAncorCommand() uses os.Executable() on all platforms — command
		// will be the real absolute path in tests, not bare "ancora".
		if !strings.Contains(text, "command = ") || !strings.Contains(text, "ancora") {
			t.Fatalf("expected ancora command in config, got:\n%s", text)
		}
		if !strings.Contains(text, `args = ["mcp", "--tools=agent"]`) {
			t.Fatalf("expected ancora args in config, got:\n%s", text)
		}
		instructionsPath := filepath.Join(home, ".codex", "ancora-instructions.md")
		if !strings.Contains(text, "model_instructions_file = \""+instructionsPath+"\"") {
			t.Fatalf("expected model_instructions_file in config, got:\n%s", text)
		}
		compactPromptPath := filepath.Join(home, ".codex", "ancora-compact-prompt.md")
	...
	instructionsRaw, err := os.ReadFile(filepath.Join(home, ".codex", "ancora-instructions.md"))
	if err != nil {
	...
	compactRaw, err := os.ReadFile(filepath.Join(home, ".codex", "ancora-compact-prompt.md"))
	if err != nil {
...
func TestInstallOpenCodeSuccessAndMCPRegistered(t *testing.T) {
	...
	pluginPath := filepath.Join(xdg, "opencode", "plugins", "ancora.ts")
	...
	if _, ok := mcp["ancora"]; !ok {
		t.Fatalf("expected mcp.ancora registration")
	}
}
...
func TestInstallOpenCodeReadEmbeddedError(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "read embedded ancora.ts") {
...
func TestInjectOpenCodeMCPPreservesExistingAndIsIdempotent(t *testing.T) {
	...
	ancora, ok := mcp["ancora"].(map[string]any)
	if !ok {
		t.Fatalf("expected ancora object")
	}
	if ancora["enabled"] != true {
		t.Fatalf("expected ancora.enabled=true")
	}
}
...
func TestInjectOpenCodeMCPConfigErrors(t *testing.T) {
	...
	t.Run("marshal ancora entry error", func(t *testing.T) {
		resetSetupSeams(t)
		home := useTestHome(t)
		runtimeGOOS = "linux"
		xdg := filepath.Join(home, "xdg")
		...
		err := injectOpenCodeMCP()
		if err == nil || !strings.Contains(err.Error(), "marshal ancora entry") {
			t.Fatalf("expected marshal ancora entry error, got %v", err)
		}
	})
...
func TestInstallClaudeCodeBranches(t *testing.T) {
	...
	t.Run("marketplace already then install success", func(t *testing.T) {
		...
		runCommand = func(_ string, args ...string) ([]byte, error) {
			...
			if strings.Join(args, " ") != "plugin install ancora" {
...
func TestWriteClaudeCodeUserMCP(t *testing.T) {
	t.Run("writes json with absolute binary path", func(t *testing.T) {
		resetSetupSeams(t)
		home := useTestHome(t)
		osExecutable = func() (string, error) { return "/usr/local/bin/ancora", nil }

		if err := writeClaudeCodeUserMCP(); err != nil {
			t.Fatalf("writeClaudeCodeUserMCP failed: %v", err)
		}

		mcpPath := filepath.Join(home, ".claude", "mcp", "ancora.json")
		...
		if cfg["command"] != "/usr/local/bin/ancora" {
	...
	t.Run("overwrites existing (idempotent — always refreshes path)", func(t *testing.T) {
		resetSetupSeams(t)
		home := useTestHome(t)
		osExecutable = func() (string, error) { return "/new/path/ancora", nil }

		mcpDir := filepath.Join(home, ".claude", "mcp")
		if err := os.MkdirAll(mcpDir, 0755); err != nil {
			t.Fatalf("mkdir: %v", err)
		}
		if err := os.WriteFile(filepath.Join(mcpDir, "ancora.json"), []byte(`{"command":"old"}`), 0644); err != nil {
		...
		raw, err := os.ReadFile(filepath.Join(mcpDir, "ancora.json"))
		if err != nil {
			t.Fatalf("read updated config: %v", err)
		}
		var cfg map[string]any
		...
		if cfg["command"] != "/new/path/ancora" {
	...
	t.Run("marshal error returns error", func(t *testing.T) {
		resetSetupSeams(t)
		useTestHome(t)
		osExecutable = func() (string, error) { return "/bin/ancora", nil }
		jsonMarshalIndentFn = func(any, string, string) ([]byte, error) {
	...
	t.Run("write error returns error", func(t *testing.T) {
		...
		home := useTestHome(t)
		osExecutable = func() (string, error) { return "/bin/ancora", nil }
		// Make ~/.claude/mcp/ancora.json a directory so write fails
		mcpDir := filepath.Join(home, ".claude", "mcp")
		if err := os.MkdirAll(mcpDir, 0755); err != nil {
			t.Fatalf("mkdir: %v", err)
		}
		if err := os.MkdirAll(filepath.Join(mcpDir, "ancora.json"), 0755); err != nil {
...
func TestResolveAncoraCommand(t *testing.T) {
	t.Run("unix returns absolute path from os.Executable", func(t *testing.T) {
		resetSetupSeams(t)
		runtimeGOOS = "linux"
		osExecutable = func() (string, error) { return "/usr/local/bin/ancora", nil }

		got := resolveAncorCommand()
		// EvalSymlinks on a non-existent path returns an error, so the result
		// is the raw os.Executable() value.
		if got == "ancora" {
			t.Fatalf("expected absolute path on unix, got bare 'ancora'")
		}
		if !strings.Contains(got, "ancora") {
			t.Fatalf("expected ancora in path, got %q", got)
		}
	})
	...
	t.Run("darwin returns absolute path from os.Executable", func(t *testing.T) {
		resetSetupSeams(t)
		runtimeGOOS = "darwin"
		osExecutable = func() (string, error) { return "/opt/homebrew/bin/ancora", nil }

		got := resolveAncorCommand()
		if got == "ancora" {
			t.Fatalf("expected absolute path on darwin, got bare 'ancora'")
		}
		if !strings.Contains(got, "ancora") {
			t.Fatalf("expected ancora in path, got %q", got)
		}
	...
	t.Run("windows returns absolute path", func(t *testing.T) {
		resetSetupSeams(t)
		runtimeGOOS = "windows"
		osExecutable = func() (string, error) { return `C:\Users\user\bin\ancora.exe`, nil }

		got := resolveAncorCommand()
		// EvalSymlinks may change the path on real OS but in tests it should
		// either equal the input or the resolved form — either way not bare "ancora"
		if got == "ancora" {
			t.Fatalf("expected absolute path on windows, got bare 'ancora'")
		}
		if !strings.Contains(got, "ancora") {
			t.Fatalf("expected ancora in path, got %q", got)
		}
	...
	t.Run("executable error falls back to bare name on all platforms", func(t *testing.T) {
		for _, goos := range []string{"linux", "darwin", "windows"} {
			t.Run(goos, func(t *testing.T) {
				...
				if got := resolveAncorCommand(); got != "ancora" {
					t.Fatalf("expected fallback to bare 'ancora', got %q", got)
				}
			})
		}
	})
...
func TestClaudeCodeMCPDirPaths(t *testing.T) {
	...
	expectedPath := filepath.Join("/home/tester", ".claude", "mcp", "ancora.json")
...
func TestGeminiInjectUsesAbsolutePath(t *testing.T) {
	for _, tc := range []struct {
		...
		{"windows", `C:\Users\user\bin\ancora.exe`},
		{"linux", "/usr/local/bin/ancora"},
		{"darwin", "/opt/homebrew/bin/ancora"},
	} {
		t.Run(tc.goos+" uses absolute path", func(t *testing.T) {
			...
			ancora := mcpServers["ancora"].(map[string]any)
			cmd := ancora["command"].(string)
			if cmd == "ancora" {
				t.Fatalf("expected absolute path on %s, got bare 'ancora'", tc.goos)
			}
			if !strings.Contains(cmd, "ancora") {
				t.Fatalf("expected ancora in command path, got %q", cmd)
			}
		})
	}

	t.Run("fallback to bare ancora when os.Executable fails", func(t *testing.T) {
		resetSetupSeams(t)
		runtimeGOOS = "linux"
		osExecutable = func() (string, error) { return "", errors.New("no executable") }
		...
		mcpServers := cfg["mcpServers"].(map[string]any)
		ancora := mcpServers["ancora"].(map[string]any)
		if got := ancora["command"]; got != "ancora" {
			t.Fatalf("expected bare 'ancora' fallback, got %#v", got)
		}
	})
}

// TestCodexBlockUsesAbsolutePath verifies codexAncorBlockStr() always bakes
...
func TestCodexBlockUsesAbsolutePath(t *testing.T) {
	for _, tc := range []struct {
		...
		{"windows", `C:\Users\user\bin\ancora.exe`, `C:\Users\user\bin\ancora.exe`},
		{"linux", "/usr/local/bin/ancora", "/usr/local/bin/ancora"},
		{"darwin", "/opt/homebrew/bin/ancora", "/opt/homebrew/bin/ancora"},
	} {
		t.Run(tc.goos+" uses absolute path in codex block", func(t *testing.T) {
			...
			block := codexAncorBlockStr()
			if !strings.Contains(block, "[mcp_servers.ancora]") {
				t.Fatalf("expected mcp_servers.ancora header, got:\n%s", block)
			}
			if !strings.Contains(block, `args = ["mcp", "--tools=agent"]`) {
				t.Fatalf("expected args in codex block, got:\n%s", block)
			}
			if block == codexAncorBlock {
				t.Fatalf("expected absolute path, got bare-ancora fallback block:\n%s", block)
			}
		})
	}

	t.Run("falls back to bare ancora when os.Executable fails", func(t *testing.T) {
		resetSetupSeams(t)
		runtimeGOOS = "linux"
		osExecutable = func() (string, error) { return "", errors.New("no executable") }

		block := codexAncorBlockStr()
		if !strings.Contains(block, `command = "ancora"`) {
			t.Fatalf("expected bare ancora fallback in codex block, got:\n%s", block)
		}
	})
}
...
func TestPathHelpersAcrossOSVariants(t *testing.T) {
	...
	if got := codexInstructionsPath(); got != filepath.Join(filepath.Dir(codexConfigPath()), "ancora-instructions.md") {
		t.Fatalf("unexpected codex instructions path: %s", got)
	}
	if got := codexCompactPromptPath(); got != filepath.Join(filepath.Dir(codexConfigPath()), "ancora-compact-prompt.md") {
...
func TestGeminiAndCodexHelpersErrorPaths(t *testing.T) {
	t.Run("injectGeminiMCP creates file from missing config", func(t *testing.T) {
		...
		ancora, ok := mcpServers["ancora"].(map[string]any)
		if !ok {
			t.Fatalf("expected ancora server object")
		}
		// resolveAncorCommand() now returns absolute path on all platforms.
		cmd, ok := ancora["command"].(string)
		if !ok || !strings.Contains(cmd, "ancora") {
			t.Fatalf("expected command containing 'ancora', got %#v", ancora["command"])
		}
	})
	...
	t.Run("upsertCodexAncorBlock replaces section before another section", func(t *testing.T) {
		...
		output := upsertCodexAncorBlock(input)
		if strings.Count(output, "[mcp_servers.ancora]") != 1 {
			t.Fatalf("expected one ancora block, got:\n%s", output)
		}
	...
	t.Run("upsertCodexAncorBlock from empty content", func(t *testing.T) {
		resetSetupSeams(t)
		// Force fallback path so output matches the constant.
		osExecutable = func() (string, error) { return "", errors.New("no executable") }

		output := upsertCodexAncorBlock("\n\n")
		if output != codexAncorBlock+"\n" {
			t.Fatalf("unexpected output for empty content:\n%s", output)
		}
	})
}

func TestValidateBinaryInPATH(t *testing.T) {
	...
	t.Run("binary not in PATH returns helpful error", func(t *testing.T) {
		resetSetupSeams(t)
		lookPathFn = func(name string) (string, error) {
			return "", fmt.Errorf("not found: %s", name)
		}
		_, err := ValidateBinaryInPATH()
		if err == nil {
		...
		if !strings.Contains(err.Error(), "~/.local/bin") {
			t.Errorf("expected install location hints in error, got: %v", err)
		}
	})
}
...
func TestAdditionalHelperBranches(t *testing.T) {
	...
	t.Run("writeCodexMemoryInstructionFiles compact write error", func(t *testing.T) {
		...
		compactPath := filepath.Join(home, ".codex", "ancora-compact-prompt.md")
...
func TestInjectOpenCodeMCPHandlesJSONC(t *testing.T) {
  ...
  "mcp": {
	...
	// Verify ancora was added to the .jsonc file
	...
	if _, ok := mcp["ancora"]; !ok {
		t.Fatalf("expected ancora to be registered")
	}
	if _, ok := mcp["other"]; !ok {
...
func TestInjectOpenCodeMCPUsesResolvedCommand(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.goos+" writes absolute path in command array", func(t *testing.T) {
			...
			ancora := mcp["ancora"].(map[string]any)
			cmd := ancora["command"].([]any)
			if len(cmd) == 0 {
			...
			if !strings.Contains(first, "ancora") {
				t.Fatalf("expected ancora in command path, got %q", first)
			}
	...
	t.Run("executable error falls back to bare ancora on all platforms", func(t *testing.T) {
		for _, goos := range []string{"linux", "darwin", "windows"} {
			t.Run(goos, func(t *testing.T) {
				resetSetupSeams(t)
				home := useTestHome(t)
				...
				if got := cmd[0].(string); got != "ancora" {
					t.Fatalf("expected fallback to bare 'ancora' when os.Executable fails, got %q", got)
				}
			})
		}
	})
...
func TestInstallOpenCodeWarningUsesResolvedCommand(t *testing.T) {
	for _, tc := range []struct {
		...
		{"windows", `C:\bin\ancora.exe`},
		{"linux", "/nonexistent/bin/ancora"},  // non-existent so EvalSymlinks is a no-op
		{"darwin", "/nonexistent/bin/ancora"}, // non-existent so EvalSymlinks is a no-op
	} {
		t.Run(tc.goos+" warning contains absolute path", func(t *testing.T) {
			...
			// Warning must reference the binary path — not just bare "ancora"
			if !strings.Contains(stderr, "ancora") {
				t.Fatalf("expected ancora path in warning on %s, got:\n%s", tc.goos, stderr)
			}
			// Must NOT be the bare "ancora" unquoted form (since we have an absolute path)
			if strings.Contains(stderr, `["ancora",`) {
				t.Fatalf("expected absolute path (not bare ancora) in warning message, got:\n%s", stderr)
			}
		})
	}
}

// ─── Issue #113: OpenCode plugin ANCORA_BIN bake-in ─────────────────────────

// TestPatchAncoraBINLine verifies that patchAncorBINLine() correctly rewrites
// the ANCORA_BIN constant in the plugin source to include a Bun.which() runtime
// fallback and a baked-in absolute path as the final headless fallback.
func TestPatchAncoraBINLine(t *testing.T) {
	const original = `const ANCORA_BIN = process.env.ANCORA_BIN ?? "ancora"`

	t.Run("bakes in absolute path with Bun.which intermediate fallback", func(t *testing.T) {
		result := string(patchAncorBINLine([]byte(original), "/usr/local/bin/ancora"))

		if strings.Contains(result, `?? "ancora"`) {
			t.Fatalf("original bare-ancora fallback should be replaced, got:\n%s", result)
		}
		if !strings.Contains(result, `process.env.ANCORA_BIN`) {
			t.Fatalf("must keep process.env.ANCORA_BIN as first option, got:\n%s", result)
		}
		if !strings.Contains(result, `Bun.which("ancora")`) {
			t.Fatalf("must include Bun.which fallback, got:\n%s", result)
		}
		if !strings.Contains(result, `"/usr/local/bin/ancora"`) {
			t.Fatalf("must include baked-in absolute path, got:\n%s", result)
		}
		// Verify precedence order: env var ?? Bun.which ?? absolute path
		envIdx := strings.Index(result, `process.env.ANCORA_BIN`)
		whichIdx := strings.Index(result, `Bun.which`)
		absIdx := strings.Index(result, `"/usr/local/bin/ancora"`)
	...
	t.Run("Windows path with backslashes is JSON-quoted correctly", func(t *testing.T) {
		result := string(patchAncorBINLine([]byte(original), `C:\Users\user\bin\ancora.exe`))

		// The path must appear as a properly JSON-escaped string
		if !strings.Contains(result, `Bun.which("ancora")`) {
			t.Fatalf("must include Bun.which fallback, got:\n%s", result)
		}
		if !strings.Contains(result, `ancora.exe`) {
			t.Fatalf("must include Windows binary name, got:\n%s", result)
		}
	})

	t.Run("bare ancora fallback when os.Executable failed", func(t *testing.T) {
		result := string(patchAncorBINLine([]byte(original), "ancora"))

		// When absBin=="ancora", we still add Bun.which but don't repeat "ancora" as absolute
		if !strings.Contains(result, `process.env.ANCORA_BIN`) {
			t.Fatalf("must keep process.env.ANCORA_BIN, got:\n%s", result)
		}
		if !strings.Contains(result, `Bun.which("ancora")`) {
			t.Fatalf("must include Bun.which fallback, got:\n%s", result)
		}
	...
	t.Run("does not modify source if marker is absent", func(t *testing.T) {
		src := []byte(`// already patched\nconst ANCORA_BIN = process.env.ANCORA_BIN ?? Bun.which("ancora") ?? "/bin/ancora"`)
		result := patchAncorBINLine(src, "/new/bin/ancora")
	...
	t.Run("only replaces first occurrence", func(t *testing.T) {
		doubled := original + "\n" + original
		result := string(patchAncorBINLine([]byte(doubled), "/bin/ancora"))
		// One line should be replaced, the other should remain as-is
		if strings.Count(result, `?? "ancora"`) != 1 {
			t.Fatalf("expected exactly one original line to remain, got:\n%s", result)
		}
	})
}

// TestInstallOpenCodeBakesANCORABIN verifies that installOpenCode() writes a
// plugin file where ANCORA_BIN includes the absolute binary path as a fallback,
// so the plugin works in headless/systemd environments (issue #113).
func TestInstallOpenCodeBakesANCORABIN(t *testing.T) {
	t.Run("installed plugin contains absolute path fallback", func(t *testing.T) {
		...
		pluginPath := filepath.Join(home, "xdg", "opencode", "plugins", "ancora.ts")
		...
		if !strings.Contains(content, `process.env.ANCORA_BIN`) {
			t.Fatalf("installed plugin must keep process.env.ANCORA_BIN override")
		}
		// Must have Bun.which intermediate fallback
		if !strings.Contains(content, `Bun.which("ancora")`) {
			t.Fatalf("installed plugin must include Bun.which fallback")
		}
		// Must have the baked-in absolute path
		if !strings.Contains(content, `"/usr/local/bin/ancora"`) {
		...
		srcRaw, err := openCodeReadFile("plugins/opencode/ancora.ts")
		if err != nil {
			t.Fatalf("read embedded plugin: %v", err)
		}
		if !strings.Contains(string(srcRaw), `?? "ancora"`) {
			t.Fatalf("source embedded plugin must remain unpatched")
		}
	})

	t.Run("ANCORA_BIN env var still takes precedence at runtime", func(t *testing.T) {
		// We verify by inspection: the installed plugin must use ?? so that a
		// truthy process.env.ANCORA_BIN short-circuits before Bun.which and the
		...
		// const ANCORA_BIN = process.env.ANCORA_BIN ?? Bun.which("ancora") ?? "/abs/path"
		// where process.env.ANCORA_BIN is leftmost (wins if set).
		envIdx := strings.Index(content, `process.env.ANCORA_BIN`)
		whichIdx := strings.Index(content, `Bun.which("ancora")`)
		absIdx := strings.Index(content, `"/usr/local/bin/ancora"`)
		if envIdx == -1 || whichIdx == -1 || absIdx == -1 {
			t.Fatalf("missing expected tokens in installed plugin:\n%s", content)
		}
		if !(envIdx < whichIdx && whichIdx < absIdx) {
			t.Fatalf("wrong operator precedence in ANCORA_BIN line:\n%s", content)
		}
	})

	t.Run("os.Executable fallback: Bun.which added but no double-ancora", func(t *testing.T) {
		resetSetupSeams(t)
		home := useTestHome(t)
		runtimeGOOS = "linux"
		osExecutable = func() (string, error) { return "", errors.New("no executable") }
