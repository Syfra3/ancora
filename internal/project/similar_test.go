...
func TestLevenshtein(t *testing.T) {
	tests := []struct {
		...
		{a: "ancora", b: "anora", want: 1},  // missing 'c'
		{a: "ancora", b: "ancora", want: 0}, // identical
		{a: "a", b: "b", want: 1},
...
func TestFindSimilar_CaseInsensitiveAndSubstring(t *testing.T) {
	existing := []string{"Ancora", "ancora-memory", "totally-different"}
	matches := FindSimilar("ancora", existing, 3)
	...
	if matches[0].Name != "Ancora" || matches[0].MatchType != "case-insensitive" {
		t.Errorf("first match = %+v; want {Ancora, case-insensitive}", matches[0])
	}

	// Second match should be substring
	if matches[1].Name != "ancora-memory" || matches[1].MatchType != "substring" {
		t.Errorf("second match = %+v; want {ancora-memory, substring}", matches[1])
	}
}
...
func TestFindSimilar_LevenshteinHit(t *testing.T) {
	existing := []string{"anora"} // distance 1 from "ancora" (missing 'c')
	matches := FindSimilar("ancora", existing, 2)

	if len(matches) != 1 {
		t.Fatalf("expected 1 levenshtein match, got %d: %v", len(matches), matches)
	}
	if matches[0].Name != "anora" {
		t.Errorf("match name = %q; want anora", matches[0].Name)
	}
...
func TestFindSimilar_LevenshteinBeyondMaxDistance(t *testing.T) {
	...
	// "ancora" vs "completely-different-string" is far beyond maxDistance=2
	matches := FindSimilar("ancora", existing, 2)
...
func TestFindSimilar_OrderingCaseFirst(t *testing.T) {
	...
	existing := []string{
		"ancora-legacy", // substring (contains "ancora")
		"Ancora",        // case-insensitive (different case)
		"anora",         // levenshtein distance 1 (missing 'c')
	}
	matches := FindSimilar("ancora", existing, 2)
...
func TestFindSimilar_EmptyExisting(t *testing.T) {
	matches := FindSimilar("ancora", []string{}, 3)
	if len(matches) != 0 {
...
func TestFindSimilar_ZeroMaxDistance(t *testing.T) {
	...
	matches := FindSimilar("ancora", existing, 0)
