// Package server provides the HTTP API for Ancora.
//
...
func (s *Server) Start() error {
	...
	if err != nil {
		return fmt.Errorf("ancora server: listen %s: %w", addr, err)
	}
	log.Printf("[ancora] HTTP server listening on %s", addr)
...
func (s *Server) handleExport(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("Content-Disposition", "attachment; filename=ancora-export.json")
...
func (s *Server) handleMigrateProject(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Printf("[ancora] project migration failed: %v", err)
	...
	log.Printf("[ancora] migrated project %q → %q (obs: %d, sessions: %d, prompts: %d)",
